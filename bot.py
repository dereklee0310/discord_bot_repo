...
LastEditTime: 2022-07-26 16:08:19
FilePath: \discord_bot_repo\bot.py
'''
from http import client
import nextcord
from nextcord.ext import commands # https://nextcordpy.readthedocs.io/en/stable/ext/commands/index.html
import json
import os
...
for folder in os.listdir('./cogs'):
    filename = f"{folder.lower()}.py"
    if os.path.exists(os.path.join("./cogs", folder, filename)):
        cog_table[folder] = True
...
async def load(ctx, *args): # ctx: context that include all information
    if len(args) == 0: # default: load all cogs
        ...
        for key, value in cog_table.items(): # for filename in os.listdir('./cmds'):
            if value == False:
                bot.load_extension(f'cogs.{key}.{key}')
                cog_table[key] = True
        ...
        for arg in args:
            ...
            bot.load_extension(f'cogs.{arg}.{arg}')
            cog_table[arg] = True
...
async def unload(ctx, *args):
    if len(args) == 0: # send warning
        ...
        for arg in args:
            ...
            bot.unload_extension(f'cogs.{arg}.{arg}')
            cog_table[arg] = False
...
async def reload(ctx, *args):
    if len(args) == 0: # send warning
        ...
        for arg in args:
            ...
            bot.reload_extension(f'cogs.{arg}.{arg}')
